...
export const writingPosts: WritingPost[] = [
  ...
  },
  {
    ...
    content: [
      ...
      "Failure modes get named individually, because “handle errors gracefully” is not a spec. For the BYU–Idaho Support Agent that meant separate defined behaviors for retrieval returning nothing, retrieval returning something stale, safety checks failing, and the student simply asking something out of scope. Each path ends in a designed handoff to human staff — routed with context, phrased so it feels like the system working rather than apologizing. Escalation is a feature with its own acceptance criteria, not an exception handler.",
      "The sequence I follow is the same one that carried the Support Agent from prototype to institutional beta serving 20,000+ students: intents and flows first, then retrieval, then guardrails. Get the intent taxonomy and conversation shapes agreed while everything is still cheap to change; only then let the retrieval architecture and model choices fill in behind them. Teams that start from the model end up reverse-engineering a spec from whatever the demo happens to do.",
